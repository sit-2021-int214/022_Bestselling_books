...
```
scat_plot4 <- dataset %>% mutate(Type = fct_rev(Type)) %>% ggplot(aes(x=Type)) + geom_bar(aes(color=Type)) 
scat_plot4
```
Before
![graph 4](graph5.PNG)
...
Part B 
use Power Bi
Link : https://app.powerbi.com/view?r=eyJrIjoiOTE1MDUyYjEtNzk4YS00ODRjLWJjMTYtZTEzNzFmNmI5NWVlIiwidCI6IjZmNDQzMmRjLTIwZDItNDQxZC1iMWRiLWFjMzM4MGJhNjMzZCIsImMiOjEwfQ%3D%3D
