...
## Part 4: Using command from tidyverse
```
...
1. fct_count(f, sort = FALSE, prop = FALSE)
* คือ
```
fct_count(f, sort = FALSE,
prop = FALSE) Count the
number of values with each
level. 
```
expain :
...
* คือ
```
fct_unique(f) Return the
unique values, removing
duplicates. fct_unique(f)
```
expain :
